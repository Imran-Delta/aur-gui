...
class PackageDetail:
    ...
    # download_size is only ever present on -Si (remote) output; installed_size
    # is present on both -Qi and -Si. Both are human-readable strings exactly
    # as pacman prints them (e.g. "3.45 MiB") -- use helpers.parse_size_to_bytes
    # to get an integer for arithmetic. Kept separate from `size` above rather
    # than replacing it: `size` already has callers depending on its current
    # (installed-size-wins) behavior.
