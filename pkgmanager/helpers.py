...
HELPER_COMMANDS = {
    'yay': {
        ...
        'info_many': ['yay', '-Qi', '{packages}'],
        'info_remote_many': ['yay', '-Si', '{packages}'],
    ...
    'paru': {
        ...
        'info_many': ['paru', '-Qi', '{packages}'],
        'info_remote_many': ['paru', '-Si', '{packages}'],
    ...
    'trizen': {
        ...
        'info_many': ['trizen', '-Qi', '{packages}'],
        'info_remote_many': ['trizen', '-Si', '{packages}'],
    ...
    'pikaur': {
        ...
        'info_many': ['pikaur', '-Qi', '{packages}'],
        'info_remote_many': ['pikaur', '-Si', '{packages}'],
    ...
    'pacman': {
        ...
        'info_many': ['pacman', '-Qi', '{packages}'],
        'info_remote_many': ['pacman', '-Si', '{packages}'],
...
_INFO_FIELD_MAP = {
    ...
    'installed size': 'size',
    'download size': None,  # handled separately below -- see the note there
...
def parse_info_output(raw_output: str) -> PackageDetail:
    ...
    # download size / installed size get their own fields (see the comment
    # on the 'download size' map entry above) instead of sharing 'size',
    # which -Si output would otherwise silently collide on.
    if 'download size' in raw_fields:
        detail.download_size = raw_fields['download size'] or None
    if 'installed size' in raw_fields:
        detail.installed_size = raw_fields['installed size'] or None
...
def parse_size_to_bytes(value: Optional[str]) -> Optional[int]:
    """
    Convert a pacman-style human-readable size ("3.45 MiB", "512.00 KiB",
    "0.00 B") into an integer byte count. Returns None for missing/empty
    input or anything that doesn't match the expected "<number> <unit>"
    shape, rather than raising -- callers doing a disk-space check should
    treat "couldn't parse this one" as "unknown", not a crash.

    pacman's units are binary (1 KiB = 1024 B); the "MB"/"GB"-without-i
    spellings are accepted too and treated the same way, since some AUR
    helpers print sizes slightly differently from stock pacman.
    """
    if not value:
        return None
    ...
    multiplier = _SIZE_UNIT_MULTIPLIERS.get(match.group(2).lower())
...
def parse_info_output_many(raw_output: str) -> List[PackageDetail]:
    """
    Parse the output of an -Qi/-Si call made against *multiple* package
    names at once. pacman and every supported helper separate each
    package's block with a blank line here too -- the same convention
    parse_search_output already relies on -- so this just splits on that
    and reuses parse_info_output per block instead of duplicating the
    field-parsing logic.
    """
    details: List[PackageDetail] = []
    if not raw_output or not raw_output.strip():
        return details
    for block in raw_output.strip('\n').split('\n\n'):
        if not block.strip():
            continue
        detail = parse_info_output(block)
        if detail.name:
