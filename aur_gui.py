...
Tkinter front-end for the pkgmanager library (the import name is unchanged
by the aur-gui rebrand -- only this script, the window title, and the
packaging metadata refer to "aur-gui"). Tokyo Night theme per
...
ever touches a widget or mutates app-level state. The I/O-wait watcher
thread (see permissions.watch_io_wait) follows the same rule: it only ever
posts to that same queue, never touches Tk state directly.

Run with --self-test to run read-only diagnostics against the real system
and exit, instead of launching the GUI.
"""
...
from pkgmanager import preflight
from pkgmanager.config import load_config, save_config
from pkgmanager.diagnostics import format_report, has_failures, run_diagnostics
from pkgmanager.permissions import watch_io_wait
...
class SettingsDialog(tk.Toplevel):
    ...
    def __init__(self, parent, current_helper, current_noconfirm, on_save):
        ...
        plain_button(body, "Run Diagnostics…", self._run_diagnostics).grid(
    ...
    def _run_diagnostics(self):
        # Synchronous: real (if fast) subprocess calls against the live
        # system, but this is an opt-in, occasional action, not something
        # on the hot path -- not worth a second background-thread/queue
        # wiring path just for this one dialog.
        ...
        try:
            ...
            self.config(cursor="")
        messagebox.showinfo("Diagnostics", format_report(results), parent=self)
...
class PkgManagerApp:
    def __init__(self, root):
        ...
        self.io_wait_var = tk.StringVar(value="")
        self.busy = False
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="aur-gui", bg=C["bg"], fg=C["fg"], font=(FACE, 12, "bold")).pack(side="left")
        ...
        tk.Label(bottom, textvariable=self.io_wait_var, bg=C["bg"], fg=C["warning"], font=F_SM).pack(
            side="left", padx=(10, 0))
    ...
    def _do_refresh_db(self):
        try:
            self.pm.refresh(callback=lambda line: self.event_queue.put(("log", line)),
                             pid_callback=lambda pid: self.event_queue.put(("op_pid", pid)))
    ...
    def _do_install(self, names, known_aur):
        ok, message = self._check_free_space_for(names)
        if not ok:
    ...
    def _show_info(self, detail):
        ...
        size_bits = []
        if detail.download_size:
            size_bits.append(f"Download: {detail.download_size}")
        if detail.installed_size:
            size_bits.append(f"Installed: {detail.installed_size}")
        if size_bits:
            lines.append("\n" + "   ".join(size_bits))
        if detail.depends:
    ...
    def _do_update(self):
        # Queried fresh rather than reusing self.updates_results: that cache
        # is only populated once the Updates tab has actually been visited
        # this session, and could be stale even if it has been.
        try:
            upgradable = self.pm.list_upgradable()
        except PackageManagerError as exc:
            self.event_queue.put(("error", str(exc)))
            return
        if upgradable:
            ok, message = self._check_free_space_for([p.name for p in upgradable])
            if not ok:
                self.event_queue.put(("error", message))
                return
        try:
    ...
    # ---- pre-flight checks ---------------------------------------------
    #
    # Split across two methods rather than one, and NOT run entirely
    # synchronously as the original spec had it: check_stale_lock() is a
    # stat() call and run_preflight_hook() is bounded by a timeout, so both
    # are fine to run here, on the GUI thread, before spawning the
    # background op. estimate_required_space() needs a real info_many()
    # subprocess/network call with no timeout of our own choosing (an AUR
    # helper's own latency) -- running *that* synchronously here would
    # freeze the window for however long it takes, so it's called from
    # inside the background thread instead (_check_free_space_for below),
    # as the first thing _do_install/_do_update do.

    def _run_preflight(self):
        """Lock check + optional site hook. Returns True if it's safe to
        proceed with spawning the operation."""
        lock_state = preflight.check_stale_lock()
        if lock_state == preflight.LockState.HELD:
            messagebox.showerror(
                "pacman is busy",
                "Another pacman process is currently running. Wait for it to finish and try again.")
            return False
        if lock_state == preflight.LockState.STALE:
            if not messagebox.askyesno(
                    "Stale lock detected",
                    f"{preflight.DEFAULT_LOCK_PATH} exists but no pacman process is running.\n\n"
                    "This usually means a previous run was interrupted. Remove the lock and continue?"):
                return False
            if not preflight.remove_stale_lock():
                messagebox.showerror(
                    "Couldn't remove lock",
                    "The lock file couldn't be removed (elevation was cancelled or denied).")
                return False

        hook = preflight.run_preflight_hook()
        if hook.ran and not hook.ok:
            detail = f"timed out after {preflight.DEFAULT_HOOK_TIMEOUT}s" if hook.timed_out else (hook.stderr or "no output")
            messagebox.showerror("Preflight check failed", f"Site preflight hook reported a problem:\n\n{detail}")
            return False
        return True

    def _check_free_space_for(self, package_names):
        """Runs on the background thread -- see the note above on why this
        isn't part of _run_preflight(). Returns (ok, message); message is
        only set when ok is False."""
        details = self.pm.info_many(package_names, local=False)
        estimate = preflight.estimate_required_space(details)
        if not estimate.has_enough:
            need = estimate.required_bytes / (1024 ** 3)
            have = estimate.free_bytes / (1024 ** 3)
            msg = f"Needs about {need:.1f} GiB free (including a 1 GiB safety buffer) but only {have:.1f} GiB is available."
            if estimate.unknown_packages:
                msg += f" ({len(estimate.unknown_packages)} package(s) had no size info and weren't counted.)"
            return False, msg
        return True, None
    ...
    def _watch_io_wait(self, pid):
        watch_io_wait(pid, lambda waiting: self.event_queue.put(("io_wait", waiting)))
    ...
    def _handle_event(self, kind, payload):
        if kind == "backend_ready":
            ...
            self._spawn(self._watch_io_wait, payload)
        elif kind == "io_wait":
            self.io_wait_var.set("Waiting for disk I/O…" if payload else "")
        elif kind == "op_done":
...
def _parse_args(argv):
    parser = argparse.ArgumentParser(prog='aur-gui', description='pacman/AUR helper GUI')
    parser.add_argument(
        '--self-test', action='store_true',
        help="Run read-only diagnostics against the real system (binaries, live -Q/-Qi "
             "parsing, lock state, disk space, Polkit policy) and exit, instead of "
             "launching the GUI. This is NOT the unit test suite -- that suite mocks "
             "every subprocess call by design and so can't tell you anything about this "
             "machine; this flag actually talks to pacman/the AUR helper/Polkit here.")
